...
for i in range(len(N)):
    ...
    plt.ylabel("[R](M)")
    if i ==0:
        plt.axhline(y=R_tot-Eq1,color="green",linestyle='dashed')
        plt.axhline(y=R_tot-Eq2,color="blue",linestyle='dashed')
    plt.plot(t, data_1, color="green")
    plt.plot(t, data_2,color="blue")
    plt.ticklabel_format(useOffset=False)
    plt.legend(['no flowing', 'with flowing'])
plt.suptitle("[R] v.s. t Plot, when [L_tot]=%s nM" %(L_tot_e),style="italic",fontsize=15)
