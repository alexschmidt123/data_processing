...
B2 = k_on*L_tot2+k_off
B3 = k_on*L_tot3+k_off
B4 = k_on*L_tot4+k_off
B5 = k_on*L_tot5+k_off
data1 = np.zeros((n,))
data2 = np.zeros((n,))
data3 = np.zeros((n,))
data4 = np.zeros((n,))
...
for index in range(n):
    if index == 0:
        ...
        rl2 = rl2 + (-B2*rl2+A2)*dt
        rl3 = rl3 + (-B3*rl3+A3)*dt
        rl4 = rl4 + (-B4*rl4+A4)*dt
        rl5 = rl5 + (-B5*rl5+A5)*dt
        data1[index]=rl1
        data2[index]=rl2
        data3[index]=rl3
        data4[index]=rl4
...
plt.title("RL surface concentration (mol/m^2) v.s. T (0.001s) with flowing")
